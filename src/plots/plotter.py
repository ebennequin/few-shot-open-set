...
class Plotter:
    ...
    def __init__(self, figsize=[10, 10], fontsize=12, fontfamily='sans-serif',
                 fontweight: str = 'normal', dpi: int = 200, max_col: int = 1,
                 out_extension: str = 'png', out_dir=Path('./')):
    ...
    def plot(self, **kwargs):
        ...
        for metric in self.metric_dic:
            ...
            for style, color, method in zip(cycle(styles), cycle(colors), self.metric_dic[metric]):
                ...
                ax.plot(method_dic['x'],
                        method_dic['y'],
                        label=method,
                        color=color,
                        linestyle=style)
                if 'pm' in method_dic:
                    ax.fill_between(method_dic['x'],
    ...
    def nested_default_dict(depth: int, final_type: Any, i: int = 1):
        ...
        fn = partial(Plotter.nested_default_dict, depth=depth, final_type=final_type, i=i+1)
