...
class T2T(SSLMethod):
    ...
    def update_detector(
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            for i in range(batch_size):
                if prob_sorted_index[i, 0] == support_labels[i]:
                    ...
                    y[2 * batch_size + i] = int(
                        np.random.choice(prob_sorted_index[i, 2:].cpu(), 1)
                    )
                else:
                    ...
                    while choice == support_labels[i]:
                        choice = int(
                            np.random.choice(prob_sorted_index[i, 1:].cpu(), 1)
                        )
        ...
        targets_r = torch.cat(
            [torch.empty(weak_feat_q.size(0)).fill_(i).long() for i in range(4)], dim=0
        ).to(self.device)
        ...
        if include_unballed_xm:
            ...
            y_onehot = (
                torch.zeros((2 * batch_size, num_classes)).float().to(self.device)
            )
            y = torch.zeros(2 * batch_size).long().to(self.device)
            # select the most confident class and randomly choose one from rest classes
            with torch.no_grad():
                prob_sorted_index = torch.argsort(
                    logits_cls_wq, descending=True, dim=-1
                )
            ...
            Lmu = F.binary_cross_entropy_with_logits(
                matching_score_u, torch.sigmoid(matching_score_u)
            )

            loss += Lmu
        else:
            Lmu = torch.Tensor([0.0])
    ...
    def extract_rot(self, feature_extractor, img_list):
        ...
        tensor_images = torch.stack(
            [self.val_transform(img).to(self.device) for img in img_list], 0
        )
    ...
    def __call__(self, support_images, support_labels, query_images, **kwargs):
        ...
        self.rotnet_head = torch.nn.Linear(feature_extractor.layer_dims[-1], 4).to(
            self.device
        )
        self.cmm_head = CrossModalMatchingHead(
            num_classes, feature_extractor.layer_dims[-1]
        ).to(self.device)
        grouped_parameters = (
        ...
        for i in range(self.pretrain_iter):
            ...
            detector_loss, Lmx, Lr, _ = self.update_detector(
                support_images=support_images,
                query_images=query_images,
                feature_extractor=feature_extractor,
                weak_feat_s=weak_feat_s,
                weak_feat_q=weak_feat_q,
            ...
            with torch.no_grad():
                ...
                accs.append(
                    (logits_cls_wq.argmax(-1) == kwargs["query_labels"].to(self.device))
                    .float()
                    .mean()
                    .item()
                )
