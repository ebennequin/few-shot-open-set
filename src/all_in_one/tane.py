...
class Classifier(nn.Module):
    def __init__(self, args, feat_dim, param_seman, train_weight_base=False):
        ...
        self.open_generator = OpenSetGenerater(args.n_ways, feat_dim, n_head=1, neg_gen_type=args.neg_gen_type,
                                               agg=args.agg)
    ...
    def init_representation(self, param_seman):
        ...
        self.seman = {k: nn.Parameter(torch.from_numpy(v), requires_grad=False).float().cuda() for k, v in
                      seman_dict.items()}
...
class SupportCalibrator(nn.Module):
    ...
    def forward(self, support_feat, base_weights, support_seman, base_seman):
        ...
        support_center, _, support_attn, _ = self.calibrator(support_feat, base_weights, base_mem_vis, support_seman,
                                                             base_seman)
...
class OpenSetGenerater(nn.Module):
    ...
    def forward(self, support_center, base_weights, support_seman=None, base_seman=None):
        ...
        output, attcoef, attn_score, value = self.att(support_center, base_weights, base_mem_vis, support_seman,
                                                      base_seman)  ## bs*nway*nbase
...
class FeatureNet(nn.Module):
    def __init__(self, args, restype, n_class, param_seman):
        ...
        self.cls_classifier = Classifier(args, self.feat_dim, param_seman,
                                         args.train_weight_base) if 'OpenMeta' in self.featype else nn.Linear(
            self.feat_dim, n_class)
    ...
    def open_forward(self, the_input, labels, conj_ids, base_ids, test):
        ...
        test_cosine_scores, supp_protos, fakeclass_protos, loss_cls, loss_funit = self.task_proto(
            (support_feat, query_feat, openset_feat), (supp_idx, base_ids), cls_label, test)
        cls_protos = torch.cat([supp_protos, fakeclass_protos], dim=1)
        ...
        _, supp_protos_aug, fakeclass_protos_aug, loss_cls_aug, loss_funit_aug = self.task_proto(
            (supopen_feat, openset_feat, query_feat), (open_idx, base_ids), cls_label, test)
    ...
    def task_proto(self, features, cls_ids, cls_label, test=False):
        test_cosine_scores, supp_protos, fakeclass_protos, _, funit_distance = self.cls_classifier(features, cls_ids,
                                                                                                   test)
