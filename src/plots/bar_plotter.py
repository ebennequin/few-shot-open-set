    ...
    def plot(self, **kwargs):
        ...
        for i, (metric_name, metric_dic) in enumerate(self.metric_dic.items()):
            ...
            for method_index, (method, method_dic) in enumerate(sorted_items):
                sorted_tuples = sorted(zip(method_dic['x'], method_dic['y']), key=lambda x: x[0])
                for j, (label, value) in enumerate(sorted_tuples):
                    pos = center + offsets[j]
                    value = value - bottoms[label]
                    ax.bar([pos], [value], edgecolor='white', color=colors[method_index],
                           width=0.1, bottom=[bottoms[label]], label=method)
                    bottoms[label] += value
            ax.text(center - 0.2, -0.15, rf'\textbf{{{pretty[metric_name]}}}', {'size': 12})

        ax.set_xticks([center + offset for center in range(len(metric_names)) for offset in offsets])
        ax.set_xticklabels(len(metric_names) * [pretty[x] for x in labels], rotation=45, ha='right')
