...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--prepool_transforms", type=str, nargs="+", default=["trivial"]
    )
    parser.add_argument(
        "--postpool_transforms", nargs="+", type=str, default=["l2_norm"]
    )
...
def main_worker(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    train_dataset, train_sampler, train_loader = get_classic_loader(
        args,
        args.dataset,
        shuffle=True,
        training=True,
    ...
    if main_process(args):
        ...
        metrics: Dict[str, Tensor] = {
            "train_loss": torch.zeros(
                args.epochs * logs_per_epoch, dtype=torch.float32
            ),
            "val_acc": torch.zeros(args.epochs, dtype=torch.float32),
            "val_auc": torch.zeros(args.epochs, dtype=torch.float32),
        }
    ...
    for i in tqdm(range(args.epochs)):
        ...
        if args.distributed:
            train_sampler.set_epoch(
                i
            )  # necessary per https://pytorch.org/docs/stable/data.html

        for j, (images, labels) in enumerate(tqdm(train_loader, unit="batch")):

            images, labels = images.to(rank), labels.to(rank)
            feat = feature_extractor(images, layers=layer)[layer].view(
                images.size(0), -1
            )
            ...
            if main_process(args):
                if j % args.log_freq == 0:
                    metrics["train_loss"][logs_per_epoch * i + j // args.log_freq] = (
                        loss.item() / world_size
                    )
        ...
        if main_process(args):
            ...
            logger.info(
                "Epoch {}: Val acc = {:.2f}  Val AUC = {:.2f}".format(
