...
def get_mini_imagenet_set(split, args):
    return MiniImageNet(
...
def get_tiered_imagenet_set(split, args):
...
def get_task_loader(
    args, dataset_name, n_way, n_shot, n_query, n_tasks, split="test", n_workers=12
):
    if dataset_name == "cifar":
        dataset = get_cifar_set(split, args)
    elif dataset_name == "mini_imagenet":
        dataset = get_mini_imagenet_set(split, args)
    elif dataset_name == "tiered_imagenet":
        dataset = get_tiered_imagenet_set(split, args)
    else:
...
def get_dataset(dataset_name, args, split):
    if dataset_name == "cifar":
        dataset = get_cifar_set(split, args)
    elif dataset_name == "mini_imagenet":
        dataset = get_mini_imagenet_set(split, args)
...
def get_classic_loader(args, dataset_name, split="train", batch_size=1024, n_workers=6):

    dataset = get_dataset(dataset_name, args, split)
