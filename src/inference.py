...
def main(args):
    ...
    classifier_transforms: FeatureTransform = TRANSFORMS['SequentialTransform'](classifier_transforms)
    ...
    detector_transforms: FeatureTransform = TRANSFORMS['SequentialTransform'](detector_transforms)

    if args.feature_detector in ALL_IN_ONE_METHODS:

        # ==== Prepare all in one detector ====

        feature_detectors: List[FeatureDetector] = get_modules_to_try(args, 'feature_detectors', args.feature_detector,
                                                                      ALL_IN_ONE_METHODS, 'feature_detector' in args.tune)
    ...
    if args.feature_detector in SSL_METHODS:
        ...
        feature_detectors = get_modules_to_try(args, 'feature_detectors', args.feature_detector,
                                               SSL_METHODS, 'feature_detector' in args.tune)
        proba_detectors = classifiers = [None]

    else:
        # ==== Prepare few-shot classifier ====

        classifiers: List[FewShotMethod] = get_modules_to_try(args, 'classifiers', args.classifier,
        ...
        if args.proba_detector == 'none':
            ...
            proba_detectors: List[ProbaDetector] = get_modules_to_try(args, 'proba_detectors', args.proba_detector,
                                                                      PROBA_DETECTORS, 'proba_detector' in args.tune)

    # ================ Prepare data ===================

    if args.feature_detector in SSL_METHODS:  # SSL methods require raw images and the actual model, not the features
        ...
        for i, layer in enumerate(args.layers):
            features, _, train_mean[i], train_std[i] = get_test_features(
                args.data_dir, args.backbone, args.src_dataset, args.tgt_dataset,
                args.training, args.model_source, layer
            )
    ...
    data_loader = get_task_loader(args, "test", args.tgt_dataset, args.n_way, args.n_shot,
                                  ...
                                  feature_dic)  # If feature_dic is None, this loader will return raw PIL images !
...
def detect_outliers(args, layers, classifier_transforms, detector_transforms, classifier,
    ...
    intra_task_metrics = defaultdict(lambda: defaultdict(list))  # used to monitor the average evolution of some metric \
    ...
    for task_id, (support, support_labels, query, query_labels, outliers) in enumerate(tqdm(data_loader)):
        ...
        if feature_extractor is None:  # For methods that work on features directly
            for layer in support_features:
                ...
                if len(output) == 3:  # It means the method has simultaenously done the inference part
                ...
                if args.use_filtering:  # Then we filter out before giving
                    ...
                    elif args.threshold == 'inspect':
                        X = scores.view(-1, 1).numpy()
                        clf = svm.SVC()
                        clf.fit(X, outliers.numpy())
                        thresh_svm = scores[torch.from_numpy(clf.predict(X)).bool()].min().item()
                        thresh_otsu = threshold_otsu(scores.numpy())
                        plt.hist(scores[~outliers.bool()].numpy(), bins=50, color='green', label='Inliers', alpha=0.5)
                        plt.hist(scores[outliers.bool()].numpy(), bins=50, color='red', label='Outliers', alpha=0.5)
                        plt.scatter(thresh_otsu, -1, label='Otsu', s=300, c='black', marker='x', linewidths=2)
                        plt.scatter(thresh_svm, -1, label='SVM', s=300, c='orange', marker='x', linewidths=2)
                        plt.legend()
                        plt.show()
                        plt.clf()
                        believed_inliers = (scores < thresh_otsu)
                    else:
