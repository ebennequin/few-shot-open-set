...
class SSLMethod:
    def __init__(self, args, confidence_threshold, lambda_cc, lambda_em,
                 lr, n_iter, depth_to_finetune, cons_loss, temperature,
                 xi=10.0, eps=1.0, ip=1):
        self.device = 'cuda'
    ...
    def ssl_loss(self, interm_feat_wq, logits_s, support_labels, logits_cls_wq, logits_cls_sq):
        """
        ...
        if self.cons_loss == 'fixmatch':
            ...
            for _ in range(self.ip):
                d.requires_grad_()
                pred_hat = self.classification_head(self.mid2end(interm_feat_wq + self.xi * d))
    ...
    def __call__(self, support_images, support_labels, query_images, **kwargs):
        ...
        with torch.no_grad():
            ...
            for i in range(self.cache_size):
                interm_feat_sq.append(self.start2mid([query_images[i] for i in pseudo_inliers_indexes], 'strong'))
            ...
            if not hasattr(self, 'optimizer'):  # Sometimes, methods would define it already
                ...
                self.optimizer = torch.optim.SGD([self.prototypes] + \
                                                 list(self.to_finetune.parameters()),
                                                 lr=self.lr)
        l_sup = []
        l_cons = []
        l_ems = []
        ...
        for iter_ in range(self.n_iter):
            ...
            # Compute loss
            ls, lcons, lem, full_loss, conf_mask = self.ssl_loss(interm_feat_wq=interm_feat_wq[pseudo_inliers],
                                                                 logits_s=logits_cls_ws, support_labels=support_labels,
            ...
            with torch.no_grad():
                ...
                if pseudo_inliers.sum() > 0 and conf_mask.sum() > 0:
                    pseudo_label_marg_dist = F.one_hot(kwargs['query_labels'][pseudo_inliers][conf_mask], num_classes).float().mean(0)
                    conf_samples_entropy.append(- (pseudo_label_marg_dist * torch.log(pseudo_label_marg_dist + 1e-12)).sum().item() / math.log(num_classes))
        ...
        kwargs['intra_task_metrics']['secondary_metrics']['conf_samples_entropy'].append(conf_samples_entropy)
